...
router.post("/report", verifytoken, async (req, res) => {
  ...
  try {
    const issue = await dbpool.query(
      ...
      [
        //userID,
        "353825ca-1d39-4d01-a7d3-fa07bf90db58",
        report_type,
        description,
        latitude,
        longitude,
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (validateReport.rows.length === 0) {
        return res
          .status(404)
          .json({ error: "Report not found or does not belong to the user" });
      }
